...
import { Play, Loader2, Clock, Trash2, Copy, Wand2, AlertCircle, CheckCircle2, ChevronRight, Terminal } from 'lucide-react';
...
export function QueryPlayground() {
  ...
  const handleExecute = useCallback(async () => {
    if (!query.trim()) {
      toast({ title: 'Empty Query', description: 'Please enter a GraphQL query.', variant: 'destructive' });
      return;
    }

    if (!store.endpoint) {
      toast({ title: 'No Endpoint', description: 'Connect to an endpoint first.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      try {
        ...
        toast({ title: 'Invalid Variables', description: 'Variables must be valid JSON.', variant: 'destructive' });
  ...
  }, [query, variables, store.endpoint, store.headers]);
  ...
  if (!store.endpoint || store.status === 'idle') {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <p className="text-zinc-500 mb-6">Connect to a GraphQL endpoint to use the query playground.</p>
      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button 
            ...
            size="sm"
          >
            {isExecuting ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Play className="w-3.5 h-3.5 mr-1.5" />}
            Run (Ctrl+↵)
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex gap-4">
          ...
          <Card className="flex-1 min-h-0 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 shrink-0">
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Query Editor</span>
                {lastDuration !== null && (
              ...
              <div className="flex items-center gap-1">
                <button 
                  ...
                  className={`text-[11px] px-2 py-0.5 rounded transition-colors ${
                    showVariables ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-400' : 'text-zinc-500 hover:text-zinc-700'
                  }`}
                >
                  Variables
                </button>
                ...
                {showVariables && (
                  ...
                  >
                    <div className="px-3 py-1.5 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                      <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">Variables (JSON)</span>
                    </div>
                    <textarea
                      value={variables}
        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0">
          {showHistory ? (
            <Card className="flex-1 min-h-0 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 shrink-0">
                <span className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Query History</span>
                {store.queryHistory.length > 0 && (
                  <button onClick={() => store.clearQueryHistory()} className="text-[11px] text-red-500 hover:text-red-600">
                    Clear All
                  </button>
                )}
              </div>
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-mono text-xs text-zinc-500">{item.durationMs}ms</span>
                          </div>
                          <span className="text-[10px] text-zinc-400">{formatTimeAgo(item.timestamp)}</span>
                        </div>
            ...
            <Card className="flex-1 min-h-0 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 shrink-0">
                ...
                {result && (
                  <button onClick={handleCopyResult} className="text-[11px] text-zinc-500 hover:text-zinc-700 flex items-center gap-1">
              ...
              <div className="flex-1 overflow-auto p-4">
                {result ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-zinc-400">
                    ...
                    <p className="text-xs mt-1">Press <kbd className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] border border-zinc-200 dark:border-zinc-700">Ctrl</kbd> + <kbd className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] border border-zinc-200 dark:border-zinc-700">↵</kbd></p>
                  </div>
                )}
              </div>
            </Card>
