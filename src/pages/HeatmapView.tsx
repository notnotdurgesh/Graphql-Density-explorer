...
export function HeatmapView() {
  ...
  const [drillDownTab, setDrillDownTab] = useState<'data' | 'histogram' | 'json' | 'analysis'>(
    'data'
  );
  ...
  const [compareDensityData, setCompareDensityData] = useState<
    Record<string, Record<string, number>>
  >({});
  const [compareSampleData, setCompareSampleData] = useState<
  ...
  const refreshData = useCallback(async () => {
    ...
    setDiscoveryStep('Initiating discovery...');
    try {
      const newDensityData: Record<string, Record<string, number>> = { ...store.densityData };
      const newSampleData: Record<string, Record<string, unknown>[]> = { ...store.sampleData };

      // Use a small concurrency limit (e.g., 3 at a time) to be polite to the server
      const CONCURRENCY_LIMIT = 3;
      const typesToFetch = [...store.selectedTypes];

      for (let i = 0; i < typesToFetch.length; i += CONCURRENCY_LIMIT) {
        const chunk = typesToFetch.slice(i, i + CONCURRENCY_LIMIT);
        await Promise.all(
          chunk.map(async (typeName) => {
            ...
            try {
              ...
              toast({
                title: `Failed to fetch ${typeName}`,
                description: err.message || 'Check console for details.',
                variant: 'default',
              });
              // If it never fetched before, initialize with zero density
              if (!newDensityData[typeName]) {
      ...
      store.addDensityRecord({ timestamp: Date.now(), density: newDensityData });
      toast({
        title: 'Data Refreshed',
        description: 'Latest data fetched using robust discovery.',
      });
    } catch (error) {
  ...
  }, [isRefreshing, store, calculateDensity, toast]);
    ...
    if (compareMode && compareEndpoint) {
      const fetchCompareData = async () => {
        ...
        try {
          ...
          const newSampleData: Record<string, Record<string, unknown>[]> = {};
            ...
            await Promise.all(
              chunk.map(async (typeName) => {
                ...
                try {
                  ...
                  const error = err as Error;
                  console.warn(`Skipping compare ${typeName}: ${error.message}`);
                  newSampleData[typeName] = [];
                  newDensityData[typeName] = calculateDensity([], typeDef.fields);
                }
              })
  ...
  // Calculate trends for all fields
  const fieldTrends = useMemo(() => {
    const trends: Record<
      string,
      Record<string, { delta: number; direction: 'up' | 'down' | 'neutral' }>
    > = {};
    const history = store.densityHistory;

    if (history.length < 2) return trends;

    const current = history[0].density;
    const previous = history[1].density;

    Object.keys(current).forEach((type) => {
      trends[type] = {};
      Object.keys(current[type]).forEach((field) => {
        const curVal = current[type][field];
        const prevVal = previous[type]?.[field];
  ...
  const stats = useMemo(() => {
    ...
    return {
      totalRecords,
      totalFields: allDensities.length,
      best,
      worst,
      fullFields,
      emptyFields,
  ...
  const exportPNG = async () => {
    ...
    if (!element) {
      toast({
        title: 'Export Failed',
        description: 'Could not find visualization container.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const exportSVG = () => {
    if (viewMode !== 'heatmap') {
      toast({
        title: 'Export Failed',
        description: 'SVG export is only supported for the Heatmap view.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!svgElement) {
      toast({
        title: 'Export Failed',
        description: 'Could not find SVG element.',
        variant: 'destructive',
      });
      return;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <p className="text-zinc-500 mb-6">
          Go to the Schema Explorer to select types and calculate density.
        </p>
        <Button onClick={() => navigate('/schema')} className="bg-indigo-600 hover:bg-indigo-700">
  ...
  const fieldDef = selectedCell
    ? store.schema
        .find((t) => t.name === selectedCell.type)
        ?.fields.find((f) => f.name === selectedCell.field)
    : null;
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 shrink-0">
        <div>
          ...
          <p className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider">
            {stats.totalRecords.toLocaleString()} records · {stats.totalFields} fields ·{' '}
            {Object.keys(store.densityData).length} types
          </p>
        </div>
        <div className="flex items-center gap-3">
            ...
            {(['heatmap', 'bar', 'treemap'] as const).map((mode) => {
              ...
              return (
                <Button
                  ...
                  className="h-7 px-3 text-xs gap-1.5 shadow-none"
                >
                  <Icon className="h-3.5 w-3.5" /> {mode.charAt(0).toUpperCase() + mode.slice(1)}
                </Button>
              );
            })}
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={exportSVG} disabled={viewMode !== 'heatmap'}>
      ...
      <HealthDashboard
        ...
        stats={{
          ...
          typeCount: Object.keys(store.densityData).length,
          fullFields: stats.fullFields,
      ...
      <div className="flex flex-col gap-4 flex-1 min-h-0">
            ...
            <div className="flex flex-wrap items-center gap-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    store.setDensityData({});
                    store.clearDensityHistory();
                    toast({
                      title: 'History Cleared',
                      description: 'Density data and history have been reset.',
                    });
                  }}
                  className="h-8 gap-2 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 dark:border-red-900/30 dark:text-red-400 dark:hover:bg-red-900/20"
                >
              ...
              {compareMode && (
                <div className="space-y-1.5 flex-1 min-w-[180px]">
                  ...
                  <Select value={compareEndpoint} onValueChange={setCompareEndpoint}>
                    ...
                    <SelectContent>
                      {store.savedEndpoints
                        .filter((e) => e.url !== store.endpoint)
                        .map((e) => (
                          <SelectItem key={e.url} value={e.url}>
                            {e.name || e.url}
                          </SelectItem>
                        ))}
                    </SelectContent>
              ...
              <div className="flex-1 min-w-[150px] border-l border-zinc-200 dark:border-zinc-800 pl-4">
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(to right, ${store.heatmapColors.low}, ${store.heatmapColors.medium}, ${store.heatmapColors.high})`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="w-full flex-1 min-h-0 flex flex-col" id="visualization-container">
          {isRefreshing ? (
            <div className="flex flex-1 flex-col items-center justify-center text-zinc-500 bg-zinc-50/50 dark:bg-zinc-900/50 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800 animate-in fade-in duration-500 m-4">
              <div className="relative mb-6">
                <div className="absolute inset-0 bg-blue-500/10 blur-xl rounded-full scale-150 animate-pulse" />
                <RefreshCw className="h-12 w-12 animate-spin text-blue-500 relative z-10" />
                <div className="absolute inset-0 flex items-center justify-center z-20">
                  <div className="h-3 w-3 bg-blue-600 rounded-full shadow-[0_0_10px_rgba(37,99,235,0.5)]" />
                </div>
              </div>
              <p className="text-xl font-bold text-zinc-900 dark:text-zinc-50 mb-2 tracking-tight">
                Advanced Mechanism Active
              </p>
              <div className="flex flex-col items-center gap-2 max-w-md w-full px-6">
                ...
                <p className="text-[10px] text-zinc-500 font-mono uppercase tracking-widest bg-white dark:bg-zinc-950 px-3 py-1 rounded-full border border-zinc-200 dark:border-zinc-800 shadow-sm animate-pulse">
                  {discoveryStep || 'Probing Schema...'}
                </p>
              </div>
            </div>
          ) : compareMode ? (
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 flex-1 min-h-0">
              <div className="space-y-2 flex flex-col min-h-0">
                <h3 className="font-semibold text-sm text-zinc-500 shrink-0">
                  Primary: {store.endpoint}
                </h3>
                <Heatmap
                  data={filteredData}
              ...
              <div className="space-y-2 flex flex-col min-h-0">
                <h3 className="font-semibold text-sm text-zinc-500 shrink-0">
                  Compare: {compareEndpoint || 'Select endpoint'}
                </h3>
                {isFetchingCompare ? (
                ...
                >
                  <RechartsTooltip
                    ...
                    formatter={(value: number, name: string, props: any) => [
                      `${props.payload.density?.toFixed(1)}%`,
                      `${props.payload.type}.${props.payload.field}`,
                    ]}
                  />
                </Treemap>
              </ResponsiveContainer>
            ...
            <DialogDescription className="flex items-center gap-4">
              <span>
                ...
                <span className="font-bold text-zinc-900 dark:text-zinc-50">
                  {selectedCell &&
                    store.densityData[selectedCell.type]?.[selectedCell.field]?.toFixed(1)}
                  %
                </span>
              </span>
              {fieldDef && (
                <>
                  ...
                  <span>
                    ...
                    <span className="font-mono text-xs">
                      {fieldDef.isList ? `[${fieldDef.type}]` : fieldDef.type}
                      {fieldDef.isRequired ? '!' : ''}
                    </span>
                  </span>
                  <span className="text-zinc-300 dark:text-zinc-700">|</span>
                  <span>
                    Kind: <span className="font-mono text-xs">{fieldDef.kind}</span>
                  </span>
                </>
              )}
            ...
            {drillDownTab === 'data' && (
              <div className="h-full overflow-auto border rounded-md">
                <Table>
                  ...
                  <TableBody>
                    {selectedCell &&
                      store.sampleData[selectedCell.type]?.slice(0, 100).map((item, idx) => {
                        const val = item[selectedCell.field];
                        ...
                        return (
                          <TableRow key={idx}>
                            ...
                            <TableCell
                              className="font-mono text-sm max-w-[400px] truncate"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="p-3 rounded-lg border border-zinc-200 dark:border-zinc-800">
                    ...
                    <div className="text-2xl font-bold">
                      {nullAnalysis.nullPercentage.toFixed(1)}%
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded text-sm font-semibold ${
                          nullAnalysis.pattern === 'all_filled'
                            ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                            : nullAnalysis.pattern === 'all_null'
                              ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                              : nullAnalysis.pattern === 'random'
                                ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                                : nullAnalysis.pattern === 'clustered'
                    ...
                    <p className="text-xs text-zinc-500 mt-2">
                      ...
                      {nullAnalysis.pattern === 'all_null' &&
                        'No records have a value for this field.'}
                      {nullAnalysis.pattern === 'random' &&
                        'Null values are distributed randomly throughout the dataset.'}
                      {nullAnalysis.pattern === 'clustered' &&
                        'Null values appear in large consecutive clusters.'}
                      {nullAnalysis.pattern === 'leading' &&
                        'Null values appear at the beginning of the dataset.'}
                      {nullAnalysis.pattern === 'trailing' &&
                        'Null values appear at the end of the dataset.'}
                    </p>
                  </div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono font-medium">
                          {nullAnalysis.longestNullStreak} records
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-zinc-500">Longest valid streak</span>
                        <span className="font-mono font-medium">
                          {nullAnalysis.longestNonNullStreak} records
                        </span>
                      </div>
                    </div>
...
function CustomTreemapContent(props: any) {
  const { depth, x, y, width, height, index, name, density } = props;
  const COLORS = [
    '#10b981',
    '#3b82f6',
    '#f59e0b',
